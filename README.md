...
One-hot encoding was applied to other categorical features (`Gender`, `Married`, `Education`, `Self_Employed`, `Property_Area`) using `pd.get_dummies()` with `drop_first=True` to avoid multicollinearity. The `Loan_ID` column was excluded as it is an identifier, and `Loan_Status` was excluded as it is the target variable.
